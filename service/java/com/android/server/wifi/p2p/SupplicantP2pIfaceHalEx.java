...
import vendor.sprd.hardware.wifi.supplicant.V1_2.ISupplicantP2pIface;
 ...
 * {@hide}
 */
public class SupplicantP2pIfaceHalEx {
    private static final String TAG = "SupplicantP2pIfaceHalEx";
    ...
    public SupplicantP2pIfaceHalEx(WifiP2pMonitor monitor) {
        mMonitor = monitor;
    }
    ...
    protected ISupplicantP2pIface getP2pIfaceMockableV1_2() {
        if (mISupplicantP2pIface == null) return null;
        return ISupplicantP2pIface.castFrom(
    ...
    public boolean setParamToP2pSupplicant(String key, String value) {
        synchronized (mLock) {
            final String methodStr = "setParamToP2pSupplicant";
            if (!checkSupplicantP2pIfaceAndLogFailure(methodStr)) return false;

            SupplicantResult<Void> result = new SupplicantResult(
                    "setParamToP2pSupplicant(" + key + ", "+ value + ")");
            try {
                result.setResult(mISupplicantP2pIface.setParamToP2pSupplicant(key, value));
            } catch (RemoteException e) {
                Log.e(TAG, "ISupplicantP2pIface exception: " + e);
                supplicantServiceDiedHandler();
            }
    ...
    public int getParamToP2pSupplicant(String key) {
        synchronized (mLock) {
            final String methodStr = "getParamToP2pSupplicant";
            if (!checkSupplicantP2pIfaceAndLogFailure(methodStr)) return -1;

            SupplicantResult<Integer> result = new SupplicantResult(
                    "getParamToP2pSupplicant(" + key + ")");
            try {
                mISupplicantP2pIface.getParamToP2pSupplicant(key,
                        (SupplicantStatus status, int value) -> {
                            result.setResult(status, value);
                        });
            } catch (RemoteException e) {
                Log.e(TAG, "ISupplicantP2pIface exception: " + e);
                supplicantServiceDiedHandler();
